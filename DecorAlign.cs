...
public class DecorAlign : DecorComponent
{
	...
	public override void Apply(ref Vector3 pos, ref Quaternion rot, ref Vector3 scale)
	{
		...
		if (SlopeOffset != Vector3.get_zero() || SlopeScale != Vector3.get_one())
		{
			float slope = TerrainMeta.HeightMap.GetSlope01(pos);
			if (SlopeOffset != Vector3.get_zero())
			{
			...
			if (SlopeScale != Vector3.get_one())
			{
				Vector3 val4 = Vector3.Lerp(Vector3.get_one(), Vector3.get_one() + Quaternion.Inverse(rot) * (SlopeScale - Vector3.get_one()), slope);
				scale.x *= val4.x;
				scale.y *= val4.y;
				scale.z *= val4.z;
			}
		}
		Vector3 up = Vector3.Lerp(rot * Vector3.get_up(), normal, NormalAlignment);
		Quaternion val5 = QuaternionEx.LookRotationForcedUp(Vector3.Lerp(rot * Vector3.get_forward(), val2, GradientAlignment), up);
		rot = val5 * rot;
	}
}

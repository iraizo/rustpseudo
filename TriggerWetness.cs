...
public class TriggerWetness : TriggerBase
{
	...
	public float WorkoutWetness(Vector3 position)
	{
		...
		if (ApplyLocalHeightCheck && ((Component)this).get_transform().InverseTransformPoint(position).y < MinLocalHeight)
		{
			return 0f;
		}
		float num = Vector3Ex.Distance2D(OriginTransform.get_position(), position) / TargetCollider.get_radius();

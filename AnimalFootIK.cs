...
public class AnimalFootIK : MonoBehaviour
{
	...
	public bool GroundSample(Vector3 origin, out RaycastHit hit)
	{
		...
		if (Physics.Raycast(origin + Vector3.get_up() * 0.5f, Vector3.get_down(), ref hit, 1f, 455155969))
		{
			return true;
		}
		return false;
	...
	private void OnAnimatorIK(int layerIndex)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			Vector3.get_up();
			Vector3 position = ((Component)val).get_transform().get_position();
			float iKPositionWeight = animator.GetIKPositionWeight(val2);
			if (GroundSample(((Component)val).get_transform().get_position() - Vector3.get_down() * actualFootOffset, out var hit))
			{
				((RaycastHit)(ref hit)).get_normal();
				position = ((RaycastHit)(ref hit)).get_point();
				float num = Vector3.Distance(((Component)val).get_transform().get_position() - Vector3.get_down() * actualFootOffset, position);
				iKPositionWeight = 1f - Mathf.InverseLerp(minWeightDistance, maxWeightDistance, num);
				animator.SetIKPosition(val2, position + Vector3.get_up() * actualFootOffset);
			}
			else
			{
				iKPositionWeight = 0f;

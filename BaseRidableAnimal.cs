...
public class BaseRidableAnimal : BaseVehicle
{
	...
	public void UpdateGroundNormal(bool force = false)
	{
		...
		if (Time.get_time() >= nextGroundNormalUpdateTime || force)
		{
			...
			{
				if (TransformUtil.GetGroundInfo(array[i].get_position() + Vector3.get_up() * 2f, out var _, out var normal, 4f, LayerMask.op_Implicit(278986753)))
				{
					targetUp += normal;
				}
				else
	...
	public float ObstacleDistanceCheck(float speed = 10f)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			if (!flag)
			{
				if (!TransformUtil.GetGroundInfo(val5 + Vector3.get_up() * 2f, out pos, out normal, 2f + num7, LayerMask.op_Implicit(278986753)))
				{
					return num4;
				}
			...
			if (flag)
			{
				...
				if (num8 > maxWallClimbSlope || num9 > maxWallClimbSlope)
				{
					...
					for (int j = 0; j < normalOffsets.Length; j++)
					{
						...
						if (TransformUtil.GetGroundInfo(val8 + Vector3.get_up() * num12 + normalOffsets[j].z * ((Component)this).get_transform().get_forward(), out var pos2, out var normal2, num7 + num12, LayerMask.op_Implicit(278986753)))
						{
							num11++;
							val7 += normal2;
							num10 += pos2.y;
	...
	public bool DropToGround(Vector3 targetPos, bool force = false)
	{
		...
		if (TransformUtil.GetGroundInfo(targetPos, out var pos, out var _, range, LayerMask.op_Implicit(278986753)))
		{
			if (Physics.CheckSphere(pos + Vector3.get_up() * 1f, 0.2f, 278986753))
			{
				return false;
			}

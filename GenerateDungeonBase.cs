...
public class GenerateDungeonBase : ProceduralComponent
{
	...
	private static Vector3 VolumeExtrudePositive = Vector3.get_one() * 0.01f;

	private static Vector3 VolumeExtrudeNegative = Vector3.get_one() * -0.01f;
	...
	public override void Process(uint seed)
	{
		...
		foreach (DungeonBaseInfo item in Object.op_Implicit((Object)(object)TerrainMeta.Path) ? TerrainMeta.Path.DungeonBaseEntrances : null)
		{
			...
			foreach (TerrainPathConnect obj in componentsInChildren)
			{
				...
				segmentStart.position = ((Component)item).get_transform().get_position();
				segmentStart.rotation = ((Component)item).get_transform().get_rotation();
		...
		foreach (DungeonSegment item2 in segmentsTotal)
		{
			...
			{
				World.AddPrefab("DungeonBase", item2.prefab, item2.position, item2.rotation, Vector3.get_one());
			}
		}
	...
	private bool NeighbourMatches(List<DungeonSegment> segments, DungeonBaseTransition transition, Vector3 transitionPos, Quaternion transitionRot)
	{
		...
		foreach (DungeonSegment segment in segments)
		{
			...
			{
				val = segment.position + segment.rotation * ((Component)socket).get_transform().get_localPosition() - transitionPos;
				if (((Vector3)(ref val)).get_sqrMagnitude() < 0.01f)
				{
	...
	private int SocketMatches(List<DungeonSegment> segments, DungeonBaseLink link, Vector3 linkPos, Quaternion linkRot)
	{
		...
		foreach (DungeonSegment segment in segments)
		{
			foreach (DungeonBaseSocket socket in segment.link.Sockets)
			{
				Vector3 val = segment.position + segment.rotation * ((Component)socket).get_transform().get_localPosition();
				foreach (DungeonBaseSocket socket2 in link.Sockets)
				{
					if (!((Object)(object)socket == (Object)(object)socket2))
					{
						Vector3 val2 = linkPos + linkRot * ((Component)socket2).get_transform().get_localPosition();
						Vector3 val3 = val - val2;
						if (((Vector3)(ref val3)).get_sqrMagnitude() < 0.01f)
						{
							num++;
						}
					}
	...
	private bool IsOccupied(List<DungeonSegment> segments, DungeonBaseSocket socket, Vector3 socketPos, Quaternion socketRot)
	{
		...
		foreach (DungeonSegment segment in segments)
		{
			foreach (DungeonBaseSocket socket2 in segment.link.Sockets)
			{
				...
				{
					Vector3 val = segment.position + segment.rotation * ((Component)socket2).get_transform().get_localPosition() - socketPos;
					if (((Vector3)(ref val)).get_sqrMagnitude() < 0.01f)
					{
						return true;
					}
	...
	private bool IsBlocked(List<DungeonSegment> segments, DungeonBaseLink link, Vector3 linkPos, Quaternion linkRot)
	{
		...
		foreach (DungeonSegment segment2 in segments)
		{
			foreach (DungeonVolume volume3 in segment2.link.Volumes)
			{
				...
				{
					Vector3 val4 = linkPos + linkRot * ((Component)socket3).get_transform().get_localPosition();
					...
					{
						val3 = segment2.position + segment2.rotation * ((Component)socket4).get_transform().get_localPosition() - val4;
						if (((Vector3)(ref val3)).get_sqrMagnitude() < 0.01f)
						{
							flag2 = true;
							break;
	...
	private void PlaceSegments(ref uint seed, int count, int budget, int floors, bool attachToFemale, bool attachToMale, List<DungeonSegment> segments, Prefab<DungeonBaseLink>[] prefabs)
	{
		...
		for (int i = 0; i < segments.Count; i++)
		{
			...
			for (int j = 0; j < dungeonSegment.link.Sockets.Count; j++)
			{
				...
				Vector3 val = dungeonSegment.position + dungeonSegment.rotation * ((Component)dungeonBaseSocket).get_transform().get_localPosition();
				Quaternion val2 = dungeonSegment.rotation * ((Component)dungeonBaseSocket).get_transform().get_localRotation();
				...
				foreach (Prefab<DungeonBaseLink> prefab in prefabs)
				{
					...
					Vector3 linkPos = Vector3.get_zero();
					Quaternion linkRot = Quaternion.get_identity();
					int linkScore = 0;
	...
	private bool Place(ref uint seed, List<DungeonSegment> segments, DungeonBaseSocket targetSocket, Vector3 targetPos, Quaternion targetRot, Prefab<DungeonBaseLink> prefab, Quaternion[] rotations, out DungeonBaseSocket linkSocket, out Vector3 linkPos, out Quaternion linkRot, out int linkScore)
	{
		...
		linkSocket = null;
		linkPos = Vector3.get_one();
		linkRot = Quaternion.get_identity();
		linkScore = 0;
		DungeonBaseLink component = prefab.Component;
		...
		for (int i = 0; i < component.Sockets.Count; i++)
		{
			...
			{
				Quaternion val2 = Quaternion.FromToRotation(-((Component)dungeonBaseSocket).get_transform().get_forward(), targetRot * Vector3.get_forward());
				if (dungeonBaseSocket.Type != DungeonBaseSocketType.Vertical)
				{
					val2 = QuaternionEx.LookRotationForcedUp(val2 * Vector3.get_forward(), Vector3.get_up());
				}
				Quaternion val3 = val * val2;
				Vector3 val4 = targetPos - val3 * ((Component)dungeonBaseSocket).get_transform().get_localPosition();
				if (!IsBlocked(segments, component, val4, val3))
				{
					int num2 = SocketMatches(segments, component, val4, val3);
					if (num2 > linkScore && prefab.CheckEnvironmentVolumesOutsideTerrain(val4, val3, Vector3.get_one(), EnvironmentType.UnderwaterLab, 1f))
					{
	...
	public static void SetupAI()
	{
		...
		foreach (DungeonBaseInfo dungeonBaseEntrance in TerrainMeta.Path.DungeonBaseEntrances)
		{
			...
			val.get_transform().set_position(((Component)dungeonBaseEntrance).get_gameObject().get_transform().get_position());
			AIInformationZone aIInformationZone = AIInformationZone.Merge(list, val);
			aIInformationZone.ShouldSleepAI = true;
			val.get_transform().SetParent(((Component)dungeonBaseEntrance).get_gameObject().get_transform());
			GameObject val2 = new GameObject("WakeTrigger");
			val2.get_transform().set_position(val.get_transform().get_position() + ((Bounds)(ref aIInformationZone.bounds)).get_center());
			val2.get_transform().set_localScale(((Bounds)(ref aIInformationZone.bounds)).get_extents() + new Vector3(100f, 100f, 100f));
			((Collider)val2.AddComponent<BoxCollider>()).set_isTrigger(true);
			val2.set_layer(LayerMask.NameToLayer("Trigger"));
			val2.get_transform().SetParent(((Component)dungeonBaseEntrance).get_gameObject().get_transform());

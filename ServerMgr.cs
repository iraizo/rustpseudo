...
public class ServerMgr : SingletonComponent<ServerMgr>, IServerCallback
{
	...
	private void OnGiveUserInformation(Message packet)
	{
		...
		else if (packet.connection.protocol > 2325)
		{
			DebugEx.Log((object)string.Concat("Kicking ", packet.connection, " - their protocol is ", packet.connection.protocol, " not ", 2325), (StackTraceLogType)0);
	...
	private void UpdateServerInformation()
	{
		...
		try
		{
			...
			SteamServer.set_GameTags($"mp{ConVar.Server.maxplayers},cp{BasePlayer.activePlayerList.get_Count()},pt{Net.sv.get_ProtocolId()},qp{SingletonComponent<ServerMgr>.Instance.connectionQueue.Queued},v{2325}{text4}{text6},h{AssemblyHash},{text},{text2},{text3}");

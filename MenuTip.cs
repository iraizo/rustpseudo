...
public class MenuTip : MonoBehaviour
{
	...
	public void Update()
	{
		if (LoadingScreen.isOpen && Time.get_realtimeSinceStartup() >= nextTipTime)
		{
			currentTipIndex++;
			...
			nextTipTime = Time.get_realtimeSinceStartup() + 6f;
			UpdateTip();
		}
	}
	...
	{
		((TMP_Text)text).set_text(MenuTips[currentTipIndex].get_translated());
	}

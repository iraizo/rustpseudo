...
public class Model : MonoBehaviour, IPrefabPreProcess
{
	...
	public void BuildBoneDictionary()
	{
		if (boneDict == null)
		{
			boneDict = new BoneDictionary(((Component)this).get_transform(), boneTransforms, boneNames);
		}
	}
	...
	public Transform FindClosestBone(Vector3 worldPos)
	{
		...
		for (int i = 0; i < boneTransforms.Length; i++)
		{
			...
			{
				float num2 = Vector3.Distance(val.get_position(), worldPos);
				if (!(num2 >= num))
				{
					result = val;
					num = num2;

...
public class NPCPlayerNavigator : BaseNavigator
{
	...
	protected override bool CanUpdateMovement()
	{
		...
		if (base.CurrentNavigationType == NavigationType.NavMesh && (NPCPlayerEntity.IsDormant || !NPCPlayerEntity.syncPosition) && ((Behaviour)base.Agent).get_enabled())
		{
	...
	protected override void UpdatePositionAndRotation(Vector3 moveToPosition, float delta)
	{
		...
		if (overrideFacingDirectionMode == OverrideFacingDirectionMode.None)
		{
			if (base.CurrentNavigationType == NavigationType.NavMesh)
			{
				...
				Vector3 desiredVelocity = base.Agent.get_desiredVelocity();
				nPCPlayerEntity.SetAimDirection(((Vector3)(ref desiredVelocity)).get_normalized());
			}
			else if (base.CurrentNavigationType == NavigationType.AStar || base.CurrentNavigationType == NavigationType.Base)
			{
				NPCPlayerEntity.SetAimDirection(Vector3Ex.Direction2D(moveToPosition, ((Component)this).get_transform().get_position()));
			}
		}
	}
	...
	public override void ApplyFacingDirectionOverride()
	{
		...
		if (overrideFacingDirectionMode != 0)
		{
			...
			{
				Vector3 val = (facingDirectionOverride = GetAimDirection(NPCPlayerEntity, facingDirectionEntity));
				NPCPlayerEntity.SetAimDirection(facingDirectionOverride);
			}
		}
	}
	...
	private static Vector3 GetAimDirection(BasePlayer aimingPlayer, BaseEntity target)
	{
		...
		{
			return Vector3Ex.Direction2D(((Component)aimingPlayer).get_transform().get_position() + aimingPlayer.eyes.BodyForward() * 1000f, ((Component)aimingPlayer).get_transform().get_position());
		}
		if (Vector3Ex.Distance2D(((Component)aimingPlayer).get_transform().get_position(), ((Component)target).get_transform().get_position()) <= 0.75f)
		{
			return Vector3Ex.Direction2D(((Component)target).get_transform().get_position(), ((Component)aimingPlayer).get_transform().get_position());
		}
	...
	private static Vector3 TargetAimPositionOffset(BaseEntity target)
	{
		...
		if ((Object)(object)basePlayer != (Object)null)
		{
			...
			return basePlayer.eyes.position - Vector3.get_up() * 0.15f;
		}

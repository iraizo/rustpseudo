...
public class Muzzleflash_AlphaRandom : MonoBehaviour
{
	...
	private void OnEnable()
	{
		...
		gck[0].color = Color.get_white();
		gck[0].time = 0f;
		gck[1].color = Color.get_white();
		gck[1].time = 0.6f;
		gck[2].color = Color.get_black();
		gck[2].time = 0.75f;
		...
		foreach (ParticleSystem val in array)
		{
			...
			ColorOverLifetimeModule colorOverLifetime = val.get_colorOverLifetime();
			((ColorOverLifetimeModule)(ref colorOverLifetime)).set_color(MinMaxGradient.op_Implicit(grad));
		}
	}

...
namespace ConVar
{
	...
	public class vehicle : ConsoleSystem
	{
		...
		public static void fixcars(Arg arg)
		{
			...
			{
				if (baseVehicle.isServer && Vector3.Distance(((Component)baseVehicle).get_transform().get_position(), ((Component)basePlayer).get_transform().get_position()) <= 5f && baseVehicle.AdminFixUp(@int))
				{
					num++;
				}
			}

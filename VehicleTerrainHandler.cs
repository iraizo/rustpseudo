...
public class VehicleTerrainHandler
{
	...
	private void DoTerrainCheck()
	{
		...
		Transform transform = ((Component)vehicle).get_transform();
		RaycastHit val = default(RaycastHit);
		if (Physics.Raycast(transform.get_position() + transform.get_up() * 0.5f, -transform.get_up(), ref val, RayLength, 27328513, (QueryTriggerInteraction)1))
		{
			CurGroundPhysicsMatName = ((RaycastHit)(ref val)).get_collider().GetMaterialAt(((RaycastHit)(ref val)).get_point()).GetNameLower();
			...
			{
				if (((Object)((RaycastHit)(ref val)).get_collider()).get_name().ToLower().Contains("ice"))
				{

...
public class v_chainsaw : MonoBehaviour
{
	...
	public void OnEnable()
	{
		...
		saveST = Vector4.op_Implicit(chainRenderer.get_sharedMaterial().GetVector("_MainTex_ST"));
	}

	private void Awake()
	{
		chainlink = chainRenderer.get_sharedMaterial();
	}
	...
	private void ScrollChainTexture()
	{
		...
		float num = (chainAmount = (chainAmount + Time.get_deltaTime() * chainSpeed) % 1f);
		block.Clear();
	...
	private void Update()
	{
		...
		smokeEffect.set_enableEmission(bEngineOn);
		if (bHitMetal)
		{
			...
			{
				array[i].set_enableEmission(true);
			}
			array = hitWoodFX;
			for (int i = 0; i < array.Length; i++)
			{
				array[i].set_enableEmission(false);
			}
			array = hitFleshFX;
			for (int i = 0; i < array.Length; i++)
			{

...
public abstract class Graph : MonoBehaviour
{
	...
	protected void OnGUI()
	{
		...
		if ((int)Event.get_current().get_type() == 7 && values != null && values.Length != 0)
		{
			float num = Mathf.Max(((Rect)(ref Area)).get_width(), ScreenFill.x * (float)Screen.get_width());
			float num2 = Mathf.Max(((Rect)(ref Area)).get_height(), ScreenFill.y * (float)Screen.get_height());
			float num3 = ((Rect)(ref Area)).get_x() - Pivot.x * num + ScreenOrigin.x * (float)Screen.get_width();
			float num4 = ((Rect)(ref Area)).get_y() - Pivot.y * num2 + ScreenOrigin.y * (float)Screen.get_height();
			GL.PushMatrix();
			Material.SetPass(0);
			GL.LoadPixelMatrix();
			GL.Begin(7);

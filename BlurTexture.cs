...
public class BlurTexture : ProcessedTexture
{
	...
	public void Blur(Texture source, float radius)
	{
		...
		material.SetVector("offsets", new Vector4(radius / (float)Screen.get_width(), 0f, 0f, 0f));
		Graphics.Blit(source, val, material, 0);
		material.SetVector("offsets", new Vector4(0f, radius / (float)Screen.get_height(), 0f, 0f));

...
public class CH47FlightTest : MonoBehaviour
{
	...
	public HelicopterInputState_t GetAIInputState()
	{
		...
		HelicopterInputState_t result = default(HelicopterInputState_t);
		Vector3 val = Vector3.Cross(Vector3.get_up(), ((Component)this).get_transform().get_right());
		float num = Vector3.Dot(Vector3.Cross(Vector3.get_up(), val), Vector3Ex.Direction2D(AIMoveTarget.get_position(), ((Component)this).get_transform().get_position()));
		...
		result.roll -= ((num2 > 0f) ? 1f : 0f);
		float num3 = Vector3Ex.Distance2D(((Component)this).get_transform().get_position(), AIMoveTarget.get_position());
		float num4 = Vector3.Dot(val, Vector3Ex.Direction2D(AIMoveTarget.get_position(), ((Component)this).get_transform().get_position()));
	...
	public float GetIdealAltitude()
	{
		...
		return ((Component)AIMoveTarget).get_transform().get_position().y;
	}
	...
	public void FixedUpdate()
	{
		...
		currentThrottle = Mathf.Lerp(currentThrottle, aIInputState.throttle, 2f * Time.get_fixedDeltaTime());
		currentThrottle = Mathf.Clamp(currentThrottle, -0.2f, 1f);
		rigidBody.AddRelativeTorque(new Vector3(aIInputState.pitch * torqueScale.x, aIInputState.yaw * torqueScale.y, aIInputState.roll * torqueScale.z) * Time.get_fixedDeltaTime(), (ForceMode)0);
		avgThrust = Mathf.Lerp(avgThrust, engineThrustMax * currentThrottle, Time.get_fixedDeltaTime());
		...
		Vector3 val = Vector3.get_up() * engineThrustMax * 0.5f * currentThrottle * num2;
		Vector3 val2 = ((Component)this).get_transform().get_up() - Vector3.get_up();
		Vector3 val3 = ((Vector3)(ref val2)).get_normalized() * engineThrustMax * currentThrottle * (1f - num2);
		float num3 = rigidBody.get_mass() * (0f - Physics.get_gravity().y);
		rigidBody.AddForce(((Component)this).get_transform().get_up() * num3 * num2 * 0.99f, (ForceMode)0);
		...
		for (int i = 0; i < GroundEffects.Length; i++)
		{
			...
			if (Physics.Raycast(((Component)obj).get_transform().get_position(), Vector3.get_down(), ref val5, 50f, 8388608))
			{
				((Component)val4).get_gameObject().SetActive(true);
				((Component)val4).get_transform().set_position(((RaycastHit)(ref val5)).get_point() + new Vector3(0f, 1f, 0f));
			}
			else
			{
				((Component)val4).get_gameObject().SetActive(false);
			}
		}
	}
	...
	public void OnDrawGizmos()
	{
		...
		Gizmos.DrawSphere(((Component)AIMoveTarget).get_transform().get_position(), 1f);
		...
		Gizmos.DrawLine(((Component)this).get_transform().get_position(), ((Component)this).get_transform().get_position() + val * 10f);
		Gizmos.set_color(Color.get_red());
		Gizmos.DrawLine(((Component)this).get_transform().get_position(), ((Component)this).get_transform().get_position() + val2 * 10f);
	}

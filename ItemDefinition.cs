...
public class ItemDefinition : MonoBehaviour
{
	...
	public static ulong FindSkin(int itemID, int skinID)
	{
		...
		{
			ulong workshopDownload = itemDefinition2.get_WorkshopDownload();
			if (workshopDownload != 0L)
			{
				string itemShortName = itemDefinition2.get_ItemShortName();
				if (itemShortName == itemDefinition.shortname || itemShortName == ((Object)itemDefinition).get_name())
				{

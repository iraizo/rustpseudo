...
public class MusicTheme : ScriptableObject
{
	...
	private void OnValidate()
	{
		...
		for (int i = 0; i < clips.Count; i++)
		{
			...
			if ((Object)(object)positionedClip.musicClip != (Object)null)
			{
				...
				positionedClip.musicClip.lengthInBarsWithTail = Mathf.CeilToInt(MusicUtil.SecondsToBars(tempo, positionedClip.musicClip.audioClip.get_length()));
			}
		}
	}
	...
	public bool CanPlayInEnvironment(int currentBiome, int currentTopology, float currentRain, float currentSnow, float currentWind)
	{
		...
		if (Object.op_Implicit((Object)(object)TOD_Sky.get_Instance()) && time.Evaluate(TOD_Sky.get_Instance().Cycle.Hour) < 0f)
		{
			return false;
		}
	...
	public bool FirstClipsLoaded()
	{
		...
		{
			if ((int)firstAudioClips[i].get_loadState() != 2)
			{
				return false;
			}
		}

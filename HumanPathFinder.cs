...
public class HumanPathFinder : BasePathFinder
{
	...
	public override AIMovePoint GetBestRoamPoint(Vector3 anchorPos, Vector3 currentPos, Vector3 currentDirection, float anchorClampDistance, float lookupMaxRange = 20f)
	{
		...
		HumanNPC humanNPC;
		if ((humanNPC = npc as HumanNPC) != null)
		{
	...
	private AIMovePoint GetBestRoamPoint(AIInformationZone aiZone, Vector3 anchorPos, Vector3 currentPos, Vector3 currentDirection, float clampDistance, float lookupMaxRange)
	{
		...
		for (int i = 0; i < pointCount; i++)
		{
			...
			if (!((Component)((Component)aIPoint2).get_transform().get_parent()).get_gameObject().get_activeSelf())
			{
				continue;
			}
			float num2 = Mathf.Abs(currentPos.y - ((Component)aIPoint2).get_transform().get_position().y);
			bool flag2 = currentPos.y < WaterSystem.OceanLevel;
			if (!flag2 && ((!flag2 && ((Component)aIPoint2).get_transform().get_position().y < WaterSystem.OceanLevel) || (currentPos.y >= WaterSystem.OceanLevel && num2 > 5f)))
			{
				continue;
			}
			float num3 = 0f;
			float num4 = Vector3.Dot(currentDirection, Vector3Ex.Direction2D(((Component)aIPoint2).get_transform().get_position(), currentPos));
			...
			float num5 = Vector3.Distance(currentPos, ((Component)aIPoint2).get_transform().get_position());
			if (num5 <= 1f)
			{
				num3 -= 3000f;
			}
			if (flag)
			{
				float num6 = Vector3.Distance(anchorPos, ((Component)aIPoint2).get_transform().get_position());

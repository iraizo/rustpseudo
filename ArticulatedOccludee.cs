...
public class ArticulatedOccludee : BaseMonoBehaviour
{
	...
	private void ClearVisibility()
	{
		...
		{
			lodGroup.set_localReferencePoint(Vector3.get_zero());
	...
	private void RegisterForCulling(OcclusionCulling.Sphere sphere, bool visible)
	{
		...
		int num = OcclusionCulling.RegisterOccludee(sphere.position, sphere.radius, visible, 0.25f, isStatic: false, ((Component)this).get_gameObject().get_layer(), OnVisibilityChanged);
		if (num >= 0)
		{
	...
	public void UpdateCullingBounds()
	{
		...
		if (num > 0 && (num2 == 0 || num < num2))
		{
			...
			{
				if (renderers[i].get_isVisible())
				{
					Bounds bounds = renderers[i].get_bounds();
		...
		{
			flag = true;
			Bounds bounds2 = colliders[0].get_bounds();
			val = ((Bounds)(ref bounds2)).get_min();
			bounds2 = colliders[0].get_bounds();
			val2 = ((Bounds)(ref bounds2)).get_max();
			for (int j = 1; j < colliders.Count; j++)
			{
				Bounds bounds3 = colliders[j].get_bounds();
				Vector3 min2 = ((Bounds)(ref bounds3)).get_min();
				Vector3 max2 = ((Bounds)(ref bounds3)).get_max();
	...
	private void ApplyVisibility(bool vis)
	{
		...
		if ((Object)(object)lodGroup != (Object)null)
		{
			...
			if (num != lodGroup.get_localReferencePoint().x)
			{
				lodGroup.set_localReferencePoint(new Vector3(num, num, num));
			}
		}
	}
	...
	protected virtual void OnVisibilityChanged(bool visible)
	{
		...
		{
			float dist = Vector3.Distance(MainCamera.position, ((Component)this).get_transform().get_position());
	...
	public virtual void TriggerUpdateVisibilityBounds()
	{
		...
		{
			Vector3 val = ((Component)this).get_transform().get_position() - MainCamera.position;
			...
			else
			{
				...
				float num4 = Mathf.Lerp(71f / (339f * (float)Math.PI), 2f, num3);
				num2 = Random.Range(num4, num4 + 71f / (339f * (float)Math.PI));
			}

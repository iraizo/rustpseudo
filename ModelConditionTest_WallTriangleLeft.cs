...
public class ModelConditionTest_WallTriangleLeft : ModelConditionTest
{
	...
	public static bool CheckCondition(BaseEntity ent)
	{
		...
		for (int i = 0; i < entityLink.connections.Count; i++)
		{
			...
			{
				if (buildingBlock.blockDefinition.info.name.token == "roof" && Vector3.Angle(((Component)ent).get_transform().get_forward(), ((Component)buildingBlock).get_transform().get_forward()) < 10f)
				{
					return true;
				}
				if (buildingBlock.blockDefinition.info.name.token == "roof_triangle" && Vector3.Angle(((Component)ent).get_transform().get_forward(), ((Component)buildingBlock).get_transform().get_forward()) < 40f)
				{
					return true;
				}
			}

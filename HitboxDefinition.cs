...
public class HitboxDefinition : MonoBehaviour
{
	...
	[SerializeField]
	private Vector3 scale = Vector3.get_one();
	...
	protected virtual void OnDrawGizmosSelected()
	{
		...
		case Type.BOX:
			...
			Gizmos.set_matrix(Gizmos.get_matrix() * Matrix4x4.TRS(center, Quaternion.Euler(rotation), scale));
		...
		case Type.CAPSULE:
			Gizmos.set_matrix(((Component)this).get_transform().get_localToWorldMatrix());
			Gizmos.set_matrix(Gizmos.get_matrix() * Matrix4x4.TRS(center, Quaternion.Euler(rotation), Vector3.get_one()));
			Gizmos.set_color(Color.get_green());
			GizmosUtil.DrawWireCapsuleY(Vector3.get_zero(), scale.x, scale.y);
			Gizmos.set_color(Color.get_white());
			Gizmos.set_matrix(Matrix4x4.get_identity());
			break;
		}
	...
	protected virtual void OnDrawGizmos()
	{
		...
		case Type.BOX:
			...
			Gizmos.DrawSphere(Vector3.get_zero(), 0.005f);
			Gizmos.DrawWireCube(Vector3.get_zero(), Vector3.get_one());
			Gizmos.set_color(new Color(1f, 0f, 0f, 0.2f));
			Gizmos.DrawCube(Vector3.get_zero(), Vector3.get_one());
			Gizmos.set_color(Color.get_white());
			Gizmos.set_matrix(Matrix4x4.get_identity());
			break;

...
public class HotAirBalloon : BaseCombatEntity, SamSite.ISamSiteTarget
{
	...
	{
		//IL_0006: Unknown result type (might be due to invalid IL or missing references)
		return myRigidbody.get_velocity();
	}
	...
	public override Quaternion GetAngularVelocityServer()
	{
		...
		Vector3 angularVelocity = myRigidbody.get_angularVelocity();
		if (((Vector3)(ref angularVelocity)).get_sqrMagnitude() < 0.1f)
		{
			return Quaternion.get_identity();
		}
		return Quaternion.LookRotation(myRigidbody.get_angularVelocity(), ((Component)this).get_transform().get_up());
	}

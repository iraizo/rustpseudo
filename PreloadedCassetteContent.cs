...
public class PreloadedCassetteContent : ScriptableObject
{
	...
	public SoundDefinition GetSoundContent(uint id)
	{
		...
		{
			if (StringPool.Get(((Object)soundDefinition).get_name()) == id)
			{
		...
		{
			if (StringPool.Get(((Object)soundDefinition2).get_name()) == id)
			{
		...
		{
			if (StringPool.Get(((Object)soundDefinition3).get_name()) == id)
			{
	...
	public PreloadedCassetteContent()
		: this()
	{
	}
}

...
public class UserPersistance : IDisposable
{
	...
	public PersistantPlayer GetPlayerInfo(BasePlayer player)
	{
		PersistantPlayer val = FetchFromDatabase(player);
		if (val == null)
		{
	...
	private PersistantPlayer FetchFromDatabase(BasePlayer player)
	{
		...
		try
		{
			...
			{
				array = blueprints.QueryBlob<string>("SELECT info FROM data WHERE userid = ?", player.UserIDString);
			}
			else if (NexusServer.TryGetPlayer(player.userID, out player2) && player2.TryGetVariable("blueprints", ref val) && (int)val.get_Type() == 0)
			{
	...
	public void SetPlayerInfo(BasePlayer player, PersistantPlayer info)
	{
		...
		try
		{
			...
			if (!NexusServer.Started)
			{
				blueprints.Execute<string, byte[], int>("INSERT OR REPLACE INTO data ( userid, info, updated ) VALUES ( ?, ?, ? )", player.UserIDString, array, Epoch.get_Current());
			}
			else if (!NexusServer.TryGetPlayer(player.userID, out player2))
			{
				Debug.LogError((object)$"Couldn't find NexusPlayer to save player info! {player.userID}");
			}
			else
			{
				player2.SetVariable("blueprints", array, false, true);
			}
		}
		finally
		{

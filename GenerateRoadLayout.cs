	...
	public override void Process(uint seed)
	{
		...
		foreach (PathSegment item in list2)
		{
			...
			for (PathFinder.Node node10 = item.start; node10 != null; node10 = node10.next)
			{
				...
				{
					start2 = true;
					normX = TerrainMeta.NormalizeX(((Component)item.origin).get_transform().get_position().x);
					normZ = TerrainMeta.NormalizeZ(((Component)item.origin).get_transform().get_position().z);
				}
				else if (item.end == node10 && (Object)(object)item.target != (Object)null)
				{
					end = true;
					normX = TerrainMeta.NormalizeX(((Component)item.target).get_transform().get_position().x);
					normZ = TerrainMeta.NormalizeZ(((Component)item.target).get_transform().get_position().z);
				}

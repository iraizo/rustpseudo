...
public class VertexColorAnimator : MonoBehaviour
{
	...
	public void scrobble(float scrobblePos)
	{
		...
		Color[] array = (Color[])(object)new Color[((Component)this).GetComponent<MeshFilter>().get_sharedMesh().get_colors()
			.Length];
		int num = 0;
	...
	private void Update()
	{
		...
		if (mode == 0)
		{
			elapsedTime += Time.get_fixedDeltaTime() / timeScale;
		}
		else if (mode == 1)
		{
			elapsedTime += Time.get_fixedDeltaTime() / timeScale;
		...
		{
			if (Mathf.FloorToInt(Time.get_fixedTime() / timeScale) % 2 == 0)
			{
				elapsedTime += Time.get_fixedDeltaTime() / timeScale;
			}
			else

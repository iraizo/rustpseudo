...
public class ch47Animator : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		float num = Time.get_deltaTime() * rotorBladeSpeed * 15f;
		Vector3 localEulerAngles = frontRotorBlade.get_localEulerAngles();
		frontRotorBlade.set_localEulerAngles(new Vector3(localEulerAngles.x, localEulerAngles.y + num, localEulerAngles.z));
		localEulerAngles = rearRotorBlade.get_localEulerAngles();
		rearRotorBlade.set_localEulerAngles(new Vector3(localEulerAngles.x, localEulerAngles.y - num, localEulerAngles.z));
	}
	...
	private void EnableBlurredRotorBlades(bool enabled)
	{
		...
		{
			((Renderer)array[i]).set_enabled(enabled);
		}
		array = RotorBlades;
		for (int i = 0; i < array.Length; i++)
		{

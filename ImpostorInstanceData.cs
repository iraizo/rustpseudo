...
public class ImpostorInstanceData
{
	...
	private Vector4 positionAndScale = Vector4.get_zero();
	...
	public Vector4 PositionAndScale()
	{
		...
		{
			Transform transform = ((Component)Renderer).get_transform();
			Vector3 position = transform.get_position();
			Vector3 lossyScale = transform.get_lossyScale();
			float num = (Renderer.get_enabled() ? lossyScale.x : (0f - lossyScale.x));

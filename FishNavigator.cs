...
public class FishNavigator : BaseNavigator
{
	...
	protected override void UpdatePositionAndRotation(Vector3 moveToPosition, float delta)
	{
		...
		((Component)this).get_transform().set_position(Vector3.MoveTowards(((Component)this).get_transform().get_position(), moveToPosition, GetTargetSpeed() * delta));
		base.BaseEntity.ServerPosition = ((Component)this).get_transform().get_localPosition();
	...
	private void UpdateRotation(Vector3 moveToPosition, float delta)
	{
		...
		base.BaseEntity.ServerRotation = Quaternion.LookRotation(Vector3Ex.Direction(moveToPosition, ((Component)this).get_transform().get_position()));
	}
}

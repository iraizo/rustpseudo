...
public class CompoundBowWeapon : BowWeapon
{
	...
	{
		if (stringHoldTimeStart != 0f && Time.get_time() - stringHoldTimeStart > conditionLossHeldDelay && GetStringBonusScale() > 0f)
		{
	...
	public void RPC_StringHoldStatus(RPCMessage msg)
	{
		if (msg.read.Bit())
		{
			stringHoldTimeStart = Time.get_time();
		}
		else
		{
	...
	public float GetStringBonusScale()
	{
		...
		return Mathf.Clamp01(Mathf.Clamp01((Time.get_time() - stringHoldTimeStart) / stringHoldDurationMax) - movementPenalty);
	}

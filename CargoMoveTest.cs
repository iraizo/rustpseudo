...
public class CargoMoveTest : FacepunchBehaviour
{
	...
	public void UpdateMovement()
	{
		...
		float num = 0f;
		Vector3 val2 = val - ((Component)this).get_transform().get_position();
		...
		((Component)this).get_transform().Rotate(Vector3.get_up(), num4 * Time.get_deltaTime() * turnScale * num6, (Space)0);
		currentThrottle = Mathf.Lerp(currentThrottle, num, Time.get_deltaTime() * 0.2f);
		Transform transform = ((Component)this).get_transform();
		transform.set_position(transform.get_position() + ((Component)this).get_transform().get_forward() * 5f * Time.get_deltaTime() * currentThrottle);
		if (Vector3.Distance(((Component)this).get_transform().get_position(), val) < 60f)
		{
			targetNodeIndex++;

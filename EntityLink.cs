...
public class EntityLink : IPooled
{
	...
	public bool CanConnect(EntityLink link)
	{
		...
		return socket.CanConnect(((Component)owner).get_transform().get_position(), ((Component)owner).get_transform().get_rotation(), link.socket, ((Component)link.owner).get_transform().get_position(), ((Component)link.owner).get_transform().get_rotation());
	}
}

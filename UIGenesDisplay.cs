...
public class UIGenesDisplay : MonoBehaviour
{
	...
	public void Init(GrowableGenes genes)
	{
		...
		foreach (GrowableGene gene in genes2)
		{
			...
			{
				((Graphic)TextLinks[num - 1]).set_color(genes.Genes[num].IsPositive() ? GeneUI[num - 1].PositiveColour : GeneUI[num - 1].NegativeColour);
			}
		}
	}
	...
	private void InitFirstRow(GrowableGenes genes)
	{
		...
		foreach (GrowableGene growableGene in genes2)
		{
			...
			{
				((Behaviour)TextLinks[num - 1]).set_enabled(false);
				continue;
			}
			((Behaviour)TextLinks[num - 1]).set_enabled(true);
			((Graphic)TextLinks[num - 1]).set_color(genes.Genes[num].IsPositive() ? GeneUI[num - 1].PositiveColour : GeneUI[num - 1].NegativeColour);
		}
	}
	...
	private void InitSecondRow(GrowableGenes genes)
	{
		...
		foreach (GrowableGene growableGene in genes2)
		{
			...
			((Behaviour)TextDiagLinks[num - 1]).set_enabled(false);
			if (growableGene.Type != growableGene.PreviousType && growableGene2.Type != growableGene2.PreviousType)
			{
				((Behaviour)TextLinks[num - 1]).set_enabled(true);
				((Graphic)TextLinks[num - 1]).set_color(growableGene2.IsPositive() ? GeneUI[num - 1].PositiveColour : GeneUI[num - 1].NegativeColour);
			}
	...
	private void ShowDiagLink(int index, float rotation, GrowableGene nextGene)
	{
		...
		Vector3 localEulerAngles = ((Component)TextDiagLinks[index]).get_transform().get_localEulerAngles();
		localEulerAngles.z = rotation;
		((Component)TextDiagLinks[index]).get_transform().set_localEulerAngles(localEulerAngles);
		((Behaviour)TextDiagLinks[index]).set_enabled(true);
		((Graphic)TextDiagLinks[index]).set_color(nextGene.IsPositive() ? GeneUI[index].PositiveColour : GeneUI[index].NegativeColour);
	}

	public UIGenesDisplay()
		: this()
	{
	}
}

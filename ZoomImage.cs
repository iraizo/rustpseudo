...
public class ZoomImage : MonoBehaviour, IScrollHandler, IEventSystemHandler
{
	...
	private void Awake()
	{
		...
		((Transform)_thisTransform).set_localScale(_scale);
	}
	...
	public void OnScroll(PointerEventData eventData)
	{
		...
		RectTransformUtility.ScreenPointToLocalPointInRectangle(_thisTransform, Vector2.op_Implicit(Input.get_mousePosition()), (Camera)null, ref val);
		float y = eventData.get_scrollDelta().y;
		if (y > 0f && _scale.x < _maximumScale)
		{
			...
			thisTransform.set_anchoredPosition(thisTransform.get_anchoredPosition() - val * _scaleIncrement);
		}
		else if (y < 0f && _scale.x > _minimumScale)
		{
			...
			thisTransform2.set_anchoredPosition(thisTransform2.get_anchoredPosition() + val * _scaleIncrement);
		}
	}

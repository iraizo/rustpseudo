...
public class FileStorage : IDisposable
{
	...
	public uint Store(byte[] data, Type type, uint entityID, uint numID = 0u)
	{
		...
		try
		{
			...
			if (db != null)
			{
				db.Execute<int, byte[], int, int, int>("INSERT OR REPLACE INTO data ( crc, data, entid, filetype, part ) VALUES ( ?, ?, ?, ?, ? )", (int)cRC, data, (int)entityID, (int)type, (int)numID);
			}
			_cache.Remove(cRC);
	...
	public byte[] Get(uint crc, Type type, uint entityID, uint numID = 0u)
	{
		...
		try
		{
			...
			byte[] array = db.QueryBlob<int, int, int, int>("SELECT data FROM data WHERE crc = ? AND filetype = ? AND entid = ? AND part = ? LIMIT 1", (int)crc, (int)type, (int)entityID, (int)numID);
			if (array == null)
			{
				return null;
			}
	...
	public void Remove(uint crc, Type type, uint entityID)
	{
		...
		try
		{
			if (db != null)
			{
				db.Execute<int, int, int>("DELETE FROM data WHERE crc = ? AND filetype = ? AND entid = ?", (int)crc, (int)type, (int)entityID);
			}
			_cache.Remove(crc);
		}
		finally
	...
	public void RemoveExact(uint crc, Type type, uint entityID, uint numid)
	{
		...
		try
		{
			if (db != null)
			{
				db.Execute<int, int, int, int>("DELETE FROM data WHERE crc = ? AND filetype = ? AND entid = ? AND part = ?", (int)crc, (int)type, (int)entityID, (int)numid);
			}
			_cache.Remove(crc);
		}
		finally
	...
	public void RemoveEntityNum(uint entityid, uint numid)
	{
		...
		try
		{
			if (db != null)
			{
				db.Execute<int, int>("DELETE FROM data WHERE entid = ? AND part = ?", (int)entityid, (int)numid);
			}
	...
	internal void RemoveAllByEntity(uint entityid)
	{
		...
		try
		{
			if (db != null)
			{
				db.Execute<int>("DELETE FROM data WHERE entid = ?", (int)entityid);
			}
		}
		finally
		{
	...
	public void ReassignEntityId(uint oldId, uint newId)
	{
		...
		try
		{
			if (db != null)
			{
				db.Execute<int, int>("UPDATE data SET entid = ? WHERE entid = ?", (int)newId, (int)oldId);
			}
		}
		finally
		{

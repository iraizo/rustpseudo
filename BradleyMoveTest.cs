...
public class BradleyMoveTest : MonoBehaviour
{
	...
	public void FixedUpdate()
	{
		...
		SetDestination(followTest.get_transform().get_position());
		...
		if (num > stoppingDist)
		{
			...
			float num3 = Vector3.Dot(zero, -((Component)this).get_transform().get_right());
			float num4 = Vector3.Dot(zero, ((Component)this).get_transform().get_right());
			if (Vector3.Dot(zero, -((Component)this).get_transform().get_forward()) > num4)
			{
				if (num2 >= num3)
				{
					turning = 1f;
		...
		AdjustFriction();
		float num8 = Mathf.InverseLerp(3f, 1f, ((Vector3)(ref velocity)).get_magnitude() * Mathf.Abs(Vector3.Dot(((Vector3)(ref velocity)).get_normalized(), ((Component)this).get_transform().get_forward())));
	...
	public void SetMotorTorque(float newThrottle, bool rightSide, float torqueAmount)
	{
		...
		{
			array[i].set_motorTorque(motorTorque);
		}
	}

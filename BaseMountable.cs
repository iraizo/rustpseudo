...
public class BaseMountable : BaseCombatEntity
{
	...
	public bool ValidDismountPosition(Vector3 disPos, Vector3 visualCheckOrigin)
	{
		...
		if (!Physics.CheckCapsule(disPos + new Vector3(0f, 0.5f, 0f), disPos + new Vector3(0f, 1.3f, 0f), 0.5f, 1537286401))
		{
			Vector3 val = disPos + ((Component)this).get_transform().get_up() * 0.5f;
			RaycastHit hit2 = default(RaycastHit);
			if (IsVisible(val) && (!Physics.Linecast(visualCheckOrigin, val, ref hit2, 1486946561) || HitOurself(hit2)))
			{
				Ray val2 = new Ray(visualCheckOrigin, Vector3Ex.Direction(val, visualCheckOrigin));
				float num = Vector3.Distance(visualCheckOrigin, val);
				if (!Physics.SphereCast(val2, 0.5f, ref hit2, num, 1486946561) || HitOurself(hit2))
				{
					return true;
				}
			}
